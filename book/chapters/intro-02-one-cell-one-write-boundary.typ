...
#figure(
  table(
    ...
    [5], [data write fence], [expired lease generation commits],
...
The layers overlap deliberately. The local lane is cheap. The object-store lock
coordinates processes. The fence rejects zombies. SlateDB provides atomicity
...
`RoutedGraphCluster::write_edge` routes only to a shard owned by the local node
and requires an active lease (`src/engine/cluster.rs`). It does not forward a
write over the query TCP transport. A mutation for a remotely owned cell is
rejected, leaving remote routing to the embedding service.
