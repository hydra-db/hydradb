...
| [BFG-011](BFG-011-wal-tail-visibility-hole.md) | The compiled base plus WAL-tail merge starts its scan at `generation.last_wal_id + 1` and returns an empty overlay when no newer WAL file exists, so a commit landing in the generation's own WAL file would be in neither the base nor the tail. | **Unproven suspect** | Pending bisect | Open (2026-07-22); two `--ignored` reproduction attempts both pass. Neither is a disproof — both run on `InMemory` storage where every commit gets its own WAL file. Needs a delayed-WAL-PUT wrapper or Jepsen. | M2 snapshot read (model pending) |
| [BFG-012](BFG-012-uncoordinated-deleters-survivor-invariant.md) | Segment compaction, artifact GC and index-generation GC each check a different survivor in a different store; nothing enforces that one readable copy of a committed epoch survives. | **not-a-bug** for data loss (2026-07-22) | n/a | Guard test passes: compaction writes the live set back as a new compacted segment, so committed edges survive artifact GC deleting the only artifact. Residual **availability** risk recorded — a caller-trusted `keep_epoch` can make a later compaction hard-error. | M3 artifact/GC (model pending) |
| [BFG-013](BFG-013-compiled-generation-ahead-of-read-epoch.md) | `topology_tail_since` returns an empty overlay when `generation.base_sequence >= read_sequence`; the `>` case would merge a CSC built ahead of the pinned snapshot. | **not-a-bug** via callers (2026-07-22) | n/a | Unreachable: generation selection filters `base_sequence == base_epoch`, callers pass an artifact epoch bounded by `read_epoch`, and the refresh branch re-filters `<= read_epoch`. Guard test asserts selection never returns an ahead generation. | M2 snapshot read (model pending) |
| [BFG-014](BFG-014-unfenced-indexer-generation-gc.md) | The indexer opens as a reader and mutates `_graph_index` via object-store CAS only, bypassing SlateDB writer fencing; its generation GC takes no reader lease. | **not-a-bug** (2026-07-22) | n/a | Self-fenced: GC deletes only strictly below a freshly-read `current`, and the manifest never moves backwards, so a stale zombie deletes a strict subset — staleness makes it more conservative, not more destructive. Two guard tests pass. Corrects the "no guard at all" premise. | M3 artifact/GC (model pending) |
