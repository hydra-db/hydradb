...
| [BFG-003](BFG-003-relationship-merge-identity-scope.md) | Relationship `MERGE` coalesces by external ID while lookup is endpoint-scoped, so same-ID/different-endpoint rows can conflict before lookup. | Confirmed contract violation | Exposed by `d01e32e`; exact first bad commit pending bisect | **reproducing** (2026-07-18): approved endpoint-scoped; current code rejects (violates it). Kernel fix scoped for a reviewed PR. | M1 cell write, M5 public commands |
