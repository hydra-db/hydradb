...
sequence: confirm this process holds the writer role, take the cell's write lock, open a
serializable transaction, check the cell is not being dropped, advance the epoch, write the
edge and all its index keys, and commit atomically to the object store. We follow that
sequence for one edge, then widen to batches, bulk import, and the background job that
compacts writes into the artifacts the read path relies on.

The main file is `src/shard/write.rs`, about 5,800 lines. The single-writer machinery lives in
...
#figure(
  diagram(
    ...
    spacing: (0pt, 0.62cm),
    node((0, 0), [1. validate + `ensure_write_authority` (writer-role check)], width: 12.5cm),
    edge((0, 0), (0, 1), "->"),
...
#term("Mutation")[
  ...
  TurboLay represents the common case as an `EdgeMutation` (`src/core/model.rs:54`), which
...
- `write_edge` (`write.rs:2432`): the primary single-edge upsert. This is what a Cypher
  `MERGE` of an edge becomes.
- `create_relationship` (`write.rs:1823`): create a relationship (a Cypher `CREATE`), with
  its own id and properties.
- `delete_edge` (`write.rs:3338`) and `delete_relationship` (`write.rs:2270`).
- `write_edge_mutations_batch` (`write.rs:3846`): the primary atomic multi-edge batch.
- `ingest_edge_mutations` (`write.rs:3955`): chunks a large stream into batches (default 1024
  edges each).
- `bulk_import_edges` (`write.rs:3643`) and `bulk_append_edges_trusted` (`write.rs:3660`):
  high-volume import paths.
- `append_edge_mutation_log` (`write.rs:4025`) and `materialize_edge_mutation_log`
  (`write.rs:4160`): the two halves of the append-then-materialize path (Section 3.7).

A Cypher write reaches these from the query layer. `execute_opencypher` (`query.rs:212`) parses
the statement as a mutation; a patternless `MERGE`/`CREATE` is dispatched by
`execute_patternless_mutation` (`query.rs:1315`), which maps a single edge `MERGE` to
...
(`query.rs:1470`), so the same logical query retried produces the same key.

== The single-writer guarantee

Chapter 0 said only one writer per cell may exist. TurboLay does not rest that promise on a
...
Before anything else, `ensure_write_authority` (`lifecycle.rs:444`) checks this process is
allowed to write at all. The authority is a static, per-process role fixed when the shard is
opened, not a per-cell lease that migrates between nodes:

#srcblock("src/core/state.rs:242-246")[```rust
...
`GraphError::WriteRequiresWriter` (`error.rs:84`). A `Writer` shard resolves the role the only
way that actually matters:

#srcblock("src/shard/lifecycle.rs:449-455")[```rust
...
`self.db.writer()` returns the SlateDB writer handle, and SlateDB hands out exactly one per
database. If another process holds it, this call fails outright. There is no lease token to
compare and no fence record to read: the question "am I the writer?" is answered by SlateDB
itself, which is the same handle every write transaction is opened on.

One more check rides inside the transaction. `validate_write_fence_txn` (`lifecycle.rs:458`) —
despite the historical name, no longer a token fence — first refuses to touch a cell that is
being dropped, then re-confirms the writer role:

#srcblock("src/shard/lifecycle.rs:464-476")[```rust
...
It is a drop-guard, not an ownership arbiter. Because the drop markers are read inside the
serializable transaction (Section 3.4), a concurrent `drop_cell` that commits first forces this
write to abort rather than resurrect a cell someone is tearing down.

=== Tier 2: the object-store cell write lock

The writer role keeps the wrong process out, but within one writer process we still want to
stop two in-process callers from entering the same cell's transaction at once. That is the
object-store cell write lock: a lock object taken with a create-if-absent operation.
...
#term("Compare-and-set (CAS)")[
  ...
  cell write lock on it.
]

`acquire_cell_write_lock` (`lifecycle.rs:284`) resolves the lock's object path and calls
`acquire_distributed_write_lock` (`state.rs:351`), which loops trying to create the lock
object. `PutMode::Create` succeeds only if no lock exists; on `AlreadyExists` it tries to
reclaim the lock but only if the existing one has expired, otherwise it backs off and retries:

#srcblock("src/core/state.rs:366-414 (abridged)")[```rust
...
The lock lives at `__slatedb_graph_kernel/write_locks/<db>/<cell>` in the object store
(`lifecycle.rs:245`), and its lifetime is `GRAPH_CELL_WRITE_LOCK_TTL_MS`, five minutes
(`lib.rs:191`). `write_edge_txn` (`write.rs:2465`) takes the lock, runs the transaction body,
and releases it through `release_cell_write_lock` (`state.rs:589`), which returns the body's
result and drops the lock together.
...
serializable-snapshot isolation, and every key it reads — the drop markers, the epoch counter,
the degree counters, the idempotency record — participates in conflict detection. If any of
them changed under the write before it commits, the commit is rejected and the write retries.
The next section opens this layer in full.

#why[
  These three cover gaps the others cannot. The writer role (Tier 1) is enforced by SlateDB's
  sole-writer handle: `ensure_write_authority` for a `Writer` calls `self.db.writer()`, which
  fails unless this process holds the database's only writer, so a second process cannot even
  open a write transaction. The cell write lock (Tier 2) coordinates callers inside the writer
  and across the brief window its TTL-based reclaim allows, keeping two would-be writers from
  doing the work concurrently. The serializable transaction (Tier 3) closes the last gap: even
  if a lock's expiry let a paused holder and a fresh holder briefly overlap, both read the same
  counters and drop markers, so only one can commit and the other aborts and retries.
  Correctness rests on the transaction; the writer handle and the lock are there to make
  conflicts rare and cheap.
]
...
#term("Serializable snapshot isolation")[
  ...
  reads so the drop-guard check and counter reads participate in conflict detection.
]

The transaction is opened with `IsolationLevel::SerializableSnapshot`, and committed by
`commit_txn_strict`, which forwards the `await_durable` flag to `commit_txn_strict_with_sequence`
where it becomes SlateDB's `WriteOptions`:

#srcblock("src/codec.rs:195-199")[```rust
...
#srcblock("src/shard/write.rs:2432-2463")[```rust
...
The writer role is checked once, up front, so a `ReadOnly` shard fails before it ever takes a
permit. Inside the loop, only some errors are worth retrying. `is_retryable_write_conflict`
(`state.rs:600`) returns true for exactly two: a SlateDB serialization abort (`Slate` with
`ErrorKind::Transaction`) and cell-write-lock contention (`CellWriteConflict`). Every other
error — a validation failure, a dropped cell, a missing writer role — falls through the
`match`'s catch-all arm and returns immediately, because retrying a write that is invalid or
not permitted is pointless.
...
#srcblock("src/shard/write.rs:2693-2706")[```rust
...
(`write.rs:4558`). Relationships additionally allocate from a second counter,
`cell/<cell_id>/meta/last_relationship_id` (`keys.rs:23`), in the same transaction.

Counters are stored as big-endian `u64` (`encode_u64`, `codec.rs:269`), and an absent counter
...
#srcblock("src/shard/write.rs:2753-2808 (abridged)")[```rust
...
#table(
  ...
  [`outbox` (`keys.rs:181`)], [`.../outbox/{epoch}/{kind}/{et}/{src}/{dst}`], [the delta stream the read merge replays],
  [`owner_delta` (`keys.rs:419`)], [`.../delta_owner/{kind}/{et}/{dir}/{owner}/{epoch}/{nbr}`], [find one vertex's recent changes fast],
  [`pair_delta` (`keys.rs:455`)], [`.../delta_pair/{kind}/{et}/{src}/{dst}/{epoch}`], [find changes to one specific edge],
  [`idempotency` (`keys.rs:47`)], [`.../idem/{op}/{key}`], [detect and short-circuit a retried write],
  [`last_epoch` (`keys.rs:19`)], [`.../meta/last_epoch`], [the version stamp for the whole cell],
)
...
Phase one is `append_edge_mutation_log` (`write.rs:4025`). It bumps a separate log counter,
`cell/<cell_id>/meta/mutation_log_epoch` (`keys.rs:27`), stores the raw batch under a
`mutation_log/...` key, and writes nothing else:

#srcblock("src/shard/write.rs:4144-4155 (abridged)")[```rust
...
Phase two is `materialize_edge_mutation_log` (`write.rs:4160`). It scans the log forward from a
materialized watermark, `cell/<cell_id>/meta/mutation_log_materialized_epoch` (`keys.rs:31`),
gathers up to `GRAPH_MUTATION_LOG_MATERIALIZE_TXN_EDGES` (512, `lib.rs:195`) edges into a
...
structures the read path prefers. `build_matrix_tiles` (`artifact_build.rs:17`) confirms the
writer role via `validate_write_fence`, reads the graph with `edges_at` (the same merge the read path uses), builds the
...
#srcblock("src/codec.rs:1635-1642")[```rust
...
flight across all cells (`acquire_graph_write_permit`, `lifecycle.rs:392`). The order is always
...
prior result under `cell/<cell_id>/idem/<operation>/<key>` as its first read after the
drop-guard check. If it is present, the write returns the stored result without re-applying:
...
+ `write_edge` confirms the writer role, takes a global write permit, and locks the cell's write lane.
+ It retries, up to 32 times, a transaction that first takes the object-store cell write lock.
+ Inside the transaction it confirms the cell is not being dropped and re-checks the writer role, aborting if either fails.
