...
const CACHE_DIR_MARKER: &str = ".slatedb-graph-query-correctness-cache";
...
fn reset_dir(path: &Path) -> CheckResult<()> {
    if path.exists() {
        ...
        if has_non_marker_entries && !marker.is_file() {
            return Err(io::Error::new(
                ...
                format!(
                    "{} already exists and is not a marked correctness cache directory",
                    path.display()
                ),
            )
            .into());
    ...
    fs::write(
        ...
        b"slatedb graph query correctness cache\n",
    )?;
    Ok(())
}
