...
async fn main() -> CheckResult<()> {
    let fanouts = parse_u64_list("PHASE2_QUERY_CORRECTNESS_FANOUTS", DEFAULT_FANOUTS);
    let hops = parse_u8_list("PHASE2_QUERY_CORRECTNESS_HOPS", DEFAULT_HOPS);
    let max_hop = env_u8(
        "PHASE2_QUERY_CORRECTNESS_DATA_HOPS",
        hops.iter().copied().max().unwrap_or(20),
    );
    let repeats = env_usize("PHASE2_QUERY_CORRECTNESS_REPEATS", 3).max(1);
    let page_size = env_usize("PHASE2_QUERY_CORRECTNESS_PAGE_SIZE", 64).max(1);
    let tile_size = env_u64("PHASE2_QUERY_CORRECTNESS_MATRIX_TILE", 4_096);
    let bulk_chunk_size = env_usize("PHASE2_QUERY_CORRECTNESS_BULK_CHUNK_SIZE", 100_000);

    let root = TempCheckRoot::new()?;
    let cache_root = root.path().join("slatedb-cache");
    fs::create_dir_all(&cache_root)?;
    let (object_store, object_backend) =
        if let Ok(env_file) = std::env::var("PHASE2_QUERY_CORRECTNESS_OBJECT_ENV") {
    ...
    let run_id = format!("phase2-query-correctness-{}", std::process::id());
    println!(
...
fn graph_options(cache_dir: Option<&Path>, fanout: u64, max_hop: u8) -> GraphOpenOptions {
    ...
    GraphOpenOptions {
        limits: GraphLimits {
            ...
            max_query_runtime_ms: Some(env_u64("PHASE2_QUERY_CORRECTNESS_TIMEOUT_MS", 120_000)),
        },
        cache: cache_dir
...
impl TempCheckRoot {
    fn new() -> CheckResult<Self> {
        ...
        let path = std::env::temp_dir().join(format!(
            "phase2-query-correctness-{}-{stamp}",
            std::process::id()
        ));
        reset_dir(&path)?;
        Ok(Self { path })
