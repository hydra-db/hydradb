...
async fn main() -> ProfileResult<()> {
    let mode = string_env("PHASE0_WRITE_PROFILE_MODE", "ingest");
    let batch_size = env_usize("PHASE0_WRITE_PROFILE_BATCH_SIZE", 1_024);
    let batches = env_usize("PHASE0_WRITE_PROFILE_BATCHES", 128);
    let warmup_batches = env_usize_allow_zero("PHASE0_WRITE_PROFILE_WARMUP_BATCHES", 8);
    let seed_degree = env_u64("PHASE0_WRITE_PROFILE_SEED_DEGREE", 0);
    let seed_chunk = env_usize("PHASE0_WRITE_PROFILE_SEED_CHUNK", 50_000);
    let src = env_u64("PHASE0_WRITE_PROFILE_SRC", 1);
    let cache_bytes = env_usize("PHASE0_WRITE_PROFILE_DISK_CACHE_BYTES", 1024 * 1024 * 1024);
    let wal_flush_interval_ms = env_u64("PHASE0_WRITE_PROFILE_WAL_FLUSH_INTERVAL_MS", 1);
    let await_durable_writes = env_bool("PHASE0_WRITE_PROFILE_AWAIT_DURABLE", true);
    let trusted_chunk_size = env_usize(
        "PHASE0_WRITE_PROFILE_TRUSTED_CHUNK_SIZE",
        DEFAULT_TRUSTED_APPEND_CHUNK_EDGES,
    );
    let index_policy =
        parse_index_policy(&string_env("PHASE0_WRITE_PROFILE_INDEX_POLICY", "full"))?;
    let keep_data = env_bool("PHASE0_WRITE_PROFILE_KEEP_DATA", false);
    ...
    let shard_path = format!("phase0-write-profile-{}", std::process::id());
    ...
    if seed_degree > 0 {
        ...
        shard
            ...
            .await?;
        eprintln!(
            "phase0 write profile seed degree={} elapsed_ms={}",
            seed_degree,
    ...
    eprintln!(
        "phase0 write profile mode={mode} object_store={object_label} batch_size={batch_size} batches={batches} warmup_batches={warmup_batches} seed_degree={seed_degree} wal_flush_interval_ms={wal_flush_interval_ms} await_durable={await_durable_writes} index_policy={index_policy:?} trusted_chunk_size={trusted_chunk_size}"
    );
...
async fn run_batch(
    ...
    match mode {
        ...
        other => Err(format!("unsupported PHASE0_WRITE_PROFILE_MODE={other}").into()),
    }
}
...
fn parse_index_policy(value: &str) -> ProfileResult<GraphIndexPolicy> {
    match value.to_ascii_lowercase().as_str() {
        ...
        other => Err(format!("unsupported PHASE0_WRITE_PROFILE_INDEX_POLICY={other}").into()),
    }
}

fn object_store(root: &TempProfileRoot) -> ProfileResult<(Arc<dyn ObjectStore>, String)> {
    if let Ok(env_file) = std::env::var("PHASE0_WRITE_PROFILE_OBJECT_ENV") {
        return Ok((
