...
async fn main() -> Result<()> {
    ...
    let op_delay = parse_env_u64("GRAPH_OP_DELAY_MICROS").map(Duration::from_micros);
    ...
    match mode {
        "writer" => {
            ...
            let writer_batch = env_usize("GRAPH_WRITER_BATCH", 1_024);
            if writer_batch == 1 {
                for offset in 0..ops {
                ...
                println!(
                    "graph worker writer node={node_id} cell={cell_id} src={src} ops={ops} batch=1 epoch={}",
                ...
                println!(
                    "graph worker writer node={node_id} cell={cell_id} src={src} ops={ops} batch={writer_batch} batches={batches} inserted={inserted} existed={already_existed} epoch={end_epoch}"
                );
            }
        }
        "batch" => {
            let src = parse_env_u64("GRAPH_SRC_ID").unwrap_or(start);
            let chunk_size = env_usize("GRAPH_BULK_CHUNK", 10_000) as u64;
            let mut inserted = 0_u64;
            let mut already_existed = 0_u64;
            let mut end_epoch = shard.current_epoch(&cell_id).await?;
            ...
            println!(
                "graph worker batch node={node_id} cell={cell_id} src={src} ops={ops} inserted={inserted} existed={already_existed} epoch={end_epoch}"
            );
        }
        "chunked-batch" => {
            let src = parse_env_u64("GRAPH_SRC_ID").unwrap_or(start);
            ...
            let result = shard
                .bulk_import_edges_chunked(
                    ...
                    env_usize("GRAPH_BULK_CHUNK", 10_000),
                )
                .await?;
            println!(
                "graph worker chunked-batch node={node_id} cell={cell_id} src={src} ops={ops} inserted={} existed={} epoch={}",
                result.inserted,
        ...
        "deleter" => {
            ...
            println!(
                "graph worker deleter node={node_id} cell={cell_id} src={src} ops={ops} epoch={}",
                shard.current_epoch(&cell_id).await?
            );
        }
        "segment" => {
            let src = parse_env_u64("GRAPH_SRC_ID").unwrap_or(start);
            let chunk_size = env_usize("GRAPH_SEGMENT_CHUNK", 1_024) as u64;
            let mut inserted = 0_u64;
            let mut already_existed = 0_u64;
            let mut end_epoch = shard.current_epoch(&cell_id).await?;
            let mut chunk_start = 0_u64;
            ...
            println!(
                "graph worker segment node={node_id} cell={cell_id} src={src} ops={ops} inserted={inserted} existed={already_existed} epoch={end_epoch}"
            );
        }
        "segment-delete" => {
            let src = parse_env_u64("GRAPH_SRC_ID").unwrap_or(start);
            let stride = parse_env_u64("GRAPH_SEGMENT_DELETE_STRIDE")
                .unwrap_or(2)
                .max(1);
            ...
            println!(
                "graph worker segment-delete node={node_id} cell={cell_id} src={src} deletes={deletes} epoch={}",
                shard.current_epoch(&cell_id).await?
            );
        }
        "segment-compact" => {
            ...
            if epoch > 0 {
                ...
                let compact = shard
                    ...
                    .await?;
                println!(
                    "graph worker segment-compact node={node_id} cell={cell_id} src={src} epoch={} segments={} segment_deletes={} tombstone_deletes={} input_edges={} output_edges={}",
                ...
                println!(
                    "graph worker segment-compact node={node_id} cell={cell_id} epoch=0 skipped"
                );
            }
        }
        "segment-gc" => {
        ...
        "matrix" => {
            ...
            if epoch > 0 {
                let artifact = shard
                    ...
                    .await?;
                println!(
                    "graph worker matrix node={node_id} cell={cell_id} epoch={} edges={} out_tiles={} transpose_tiles={}",
                    artifact.base_epoch,
                    artifact.edge_count,
                    artifact.out_tiles,
        ...
        "verify" => {
            let report = shard
                .verify_current_graph(
                    &cell_id,
                    &edge_type,
                    parse_env_u8("GRAPH_VERIFY_HOPS").unwrap_or(3),
                    env_usize("GRAPH_VERIFY_ROOTS", 8),
                )
                .await?;
            println!(
        ...
        "digest" => {
            ...
            let digest = shard
                ...
                .await?;
            println!(
                "graph worker digest node={node_id} cell={cell_id} epoch={} edges={} edge_checksum={} out_degree_checksum={} in_degree_checksum={}",
                digest.read_epoch,
                digest.live_edges,
    ...
    sleep_between_ops(parse_env_u64("GRAPH_EXIT_DELAY_MICROS").map(Duration::from_micros));
    shard.close().await?;
    Ok(())
}
...
fn graph_open_options() -> GraphOpenOptions {
    let index_policy = match std::env::var("GRAPH_INDEX_POLICY") {
