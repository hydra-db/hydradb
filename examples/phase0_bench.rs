...
async fn main() -> Result<()> {
    ...
    println!(
        "phase0 hot traversal benchmark: fanouts={:?} max_hops={max_hops} requested_matrix_kernel={matrix_kernel:?} best_of_iters={bench_iters} reopen_before_warmup={reopen_before_warmup}",
        fanouts,
    );
    println!(
    ...
    for &fanout in &fanouts {
        let shard_path = format!("phase0-bench/fanout-{fanout}");
        let shard =
...
fn bench_iters() -> u32 {
    std::env::var("PHASE0_BENCH_ITERS")
        .ok()
...
fn fanouts() -> Vec<u64> {
    let Ok(value) = std::env::var("PHASE0_FANOUTS") else {
...
fn reopen_before_warmup() -> bool {
    std::env::var("PHASE0_REOPEN_BEFORE_WARMUP").is_ok_and(|value| {
        matches!(
