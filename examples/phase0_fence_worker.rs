...
use slatedb_graph_kernel::{
    ...
    GraphIndexPolicy, GraphOpenOptions, GraphShard, Result, RoutedPhase0Cluster, ShardLease,
    ShardPlacement,
};
...
async fn main() -> Result<()> {
    ...
    if args.len() < 5 {
        eprintln!(
            "usage: phase0_fence_worker <init|takeover|stale-probe|reader> <env-file|local:PATH|-> <base-path> <control-path>"
        );
        std::process::exit(2);
    }
    ...
    let lease_ttl = Duration::from_millis(env_u64("PHASE0_LEASE_TTL_MS", 5_000));
    ...
    match mode {
        "init" => {
            ...
            shard
                .rollup_artifacts(
                    ...
                    &edge_type,
                    epoch,
                    env_usize("PHASE0_POSTING_CHUNK", 4),
                    env_u64("PHASE0_MATRIX_TILE", 4),
                    env_u64("PHASE0_SUPERNODE_THRESHOLD", 2),
                    env_usize("PHASE0_SUPERNODE_CHUNK", 4),
                )
                .await?;
            cluster.close().await?;
            control.close().await?;
            println!(
                "phase0 fence init cell={cell_id} owner={} token={} epoch={epoch}",
        ...
        "takeover" => {
            ...
            control.close().await?;
            println!(
                "phase0 fence takeover cell={cell_id} failover_token={} active_token={} epoch={epoch}",
                failed_over.lease_token, active.lease_token
            );
        }
        "stale-probe" => {
            let stale_token = env_u64("PHASE0_STALE_LEASE_TOKEN", 1);
            let future_expiry = now_millis()
                .checked_add(env_u64("PHASE0_STALE_LOCAL_EXPIRY_MS", 60_000))
                .ok_or_else(|| GraphError::CorruptValue {
                    key: "phase0/stale_expiry".to_string(),
                    ...
                    .await?
            {
                return Err(GraphError::CorruptValue {
                    key: "phase0/stale_probe".to_string(),
                    reason: "stale write became visible".to_string(),
                });
            }
            shard.close().await?;
            println!(
                "phase0 fence stale-probe cell={cell_id} stale_token={stale_token} epoch={epoch} rejected_all=true"
            );
        }
        "reader" => {
            ...
            if stale_visible || !takeover_visible || !segment_visible || segment_deleted_visible {
                return Err(GraphError::CorruptValue {
                    key: "phase0/fence_reader".to_string(),
                    reason: format!(
            ...
            shard.close().await?;
            println!(
                "phase0 fence reader cell={cell_id} epoch={epoch} stale_visible={stale_visible} takeover_visible={takeover_visible}"
            );
        }
        other => {
    ...
    F: Future<Output = Result<T>>,
{
    match future.await {
        Err(GraphError::StaleShardLease {
            ...
            println!(
                "phase0 fence stale-rejected operation={operation} cell={cell_id} node={node_id} token={lease_token}"
            );
            Ok(())
        }
        Err(err) => Err(GraphError::CorruptValue {
            key: format!("phase0/fence/{operation}"),
