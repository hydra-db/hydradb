...
mod tests {
    ...
    fn parses_multi_source_native_path_call() {
        let parsed = parse_native_path_procedure(
            "CALL algo.MSpaths({sourceLabel: 'Entity', sourceProperty: 'name', sourceValues: ['alpha', 'beta'], targetValues: ['alpha', 'beta'], pairwise: true, relTypes: ['RELATES'], maxLen: 3, relDirection: 'both', pathCount: 5, resultLimit: 100}) YIELD path RETURN path",
            &BTreeMap::new(),
            10,
        )
        .unwrap()
